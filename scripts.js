...
function renderPublications(filter = 'all') {
    ...
    years.forEach(year => {
        ...
        yearHeader.className = "text-xl font-bold text-gray-500 dark:text-gray-400 border-b border-gray-200 dark:border-slate-700 pb-2 mb-4 mt-8 first:mt-0";
        ...
        yearPubs.forEach(pub => {
            ...
            pubDiv.className = "mb-6 pl-4 border-l-2 border-transparent hover:border-brand dark:hover:border-blue-400 transition duration-300";

            const formattedAuthors = pub.authors.replace(/Hoang Pham/g, "<strong class='text-gray-900 dark:text-gray-100'>Hoang Pham</strong>");
            const tagsHtml = pub.tags.map(t =>
                `<span class="inline-block bg-gray-100 dark:bg-slate-700 text-gray-600 dark:text-gray-300 text-[10px] px-2 py-0.5 rounded mr-1">${t}</span>`
            ).join('');

            const descriptionHtml = pub.description ?
                `<p class="text-sm text-gray-600 dark:text-gray-400 mt-2 leading-relaxed">${pub.description}</p>` : '';

            pubDiv.innerHTML = `
                <h4 class="text-lg font-medium text-gray-900 dark:text-gray-100 leading-tight">${pub.title}</h4>
                <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">${formattedAuthors}</p>
                <div class="flex items-center gap-2 mt-1">
                    <span class="text-sm italic text-gray-500 dark:text-gray-500">${pub.venue}</span>
                    ${tagsHtml}
                </div>
                ${descriptionHtml}
                <div class="flex gap-2 mt-2 text-xs font-bold font-sans">
                    <a href="${pub.links.pdf}" class="text-brand dark:text-blue-400 hover:underline px-0.5 py-0.5 border border-brand dark:border-blue-400 rounded hover:bg-brand dark:hover:bg-blue-600 hover:text-white transition">PDF</a>
                    <a href="${pub.links.code}" class="text-brand dark:text-blue-400 hover:underline px-0.5 py-0.5 border border-brand dark:border-blue-400 rounded hover:bg-brand dark:hover:bg-blue-600 hover:text-white transition">Code</a>
                </div>
            `;
...
function renderHome() {
    ...
    selectedPubs.forEach(pub => {
        ...
        pubDiv.innerHTML = `
            <h4 class="text-lg font-bold text-gray-900 dark:text-gray-100 group-hover:text-brand dark:group-hover:text-blue-400 transition cursor-pointer">
                <a href="#">${pub.title}</a>
            </h4>
            <div class="flex items-center gap-3 mt-1 flex-wrap">
                <p class="text-gray-500 text-sm italic">${pub.venue}</p>
                <div class="flex gap-3 text-xs font-semibold">
                    <a href="${pub.links.pdf}" class="text-brand dark:text-blue-400 hover:underline">[PDF]</a>
                    <a href="${pub.links.code}" class="text-brand dark:text-blue-400 hover:underline">[Code]</a>
                </div>
            </div>
            <p class="text-gray-600 dark:text-gray-400 text-sm mt-2">${formattedAuthors}</p>
            ${descriptionHtml}
        `;
        container.appendChild(pubDiv);
    });
...
function renderNews() {
    ...
    news.forEach((item, index) => {
        ...
        const dotColor = item.highlighted ? 'bg-brand dark:bg-blue-500' : 'bg-gray-300 dark:bg-slate-600';

        newsDiv.innerHTML = `
            <span class="absolute -left-[21px] top-1 h-3 w-3 rounded-full ${dotColor} ring-4 ring-white dark:ring-slate-900"></span>
            <p class="text-xs font-bold text-gray-500 dark:text-gray-500 uppercase tracking-wide">${item.date}</p>
            <p class="text-sm text-gray-700 dark:text-gray-300 mt-1">${item.content}</p>
        `;
...
function renderResearch() {
    ...
    if (openSourceContainer && research.openSource) {
        ...
        research.openSource.forEach(item => {
            ...
            li.innerHTML = `
                <i class="fas ${item.icon} mt-1 text-brand dark:text-blue-400 mr-3"></i>
                <div>
                    <strong class="text-gray-800 dark:text-gray-200">${item.name}</strong>
...
function renderExperience() {
    ...
    if (educationContainer && experience.education) {
        educationContainer.className = "border-l-2 border-gray-200 dark:border-slate-700 ml-5 pl-8 space-y-6";
        educationContainer.innerHTML = '';
        experience.education.forEach(edu => {
            const eduDiv = document.createElement('div');
            eduDiv.className = "relative";

            const dotColor = edu.highlighted ? 'bg-brand dark:bg-blue-500' : 'bg-gray-400 dark:bg-slate-600';
            const detailsHtml = edu.details ?
                (Array.isArray(edu.details) ?
                    edu.details.map(d => `<p class="text-sm text-gray-600 dark:text-gray-400 mt-1">${d}</p>`).join('') :
                    `<p class="text-sm text-gray-600 dark:text-gray-400 mt-1">${edu.details}</p>`) :
                '';

            eduDiv.innerHTML = `
                <span class="absolute -left-[39px] top-1 h-5 w-5 rounded-full border-4 border-white dark:border-slate-900 ${dotColor}"></span>
                <div class="flex flex-col sm:flex-row sm:justify-between sm:items-baseline">
                    <h4 class="text-lg font-bold text-gray-900 dark:text-gray-100">${edu.institution}</h4>
                    <span class="text-sm font-medium text-gray-500 dark:text-gray-400">${edu.period}</span>
                </div>
                <p class="text-gray-700 dark:text-gray-300 italic">${edu.degree}</p>
                ${detailsHtml}
            `;
    ...
    if (workContainer && experience.work) {
        ...
        experience.work.forEach(job => {
            ...
            const dotColor = job.highlighted ? 'bg-brand dark:bg-blue-500' : 'bg-gray-400 dark:bg-slate-600';

            let detailsHtml = '';
            if (job.responsibilities) {
                detailsHtml = `<ul class="list-disc list-inside text-sm text-gray-600 dark:text-gray-400 mt-2 space-y-1">${job.responsibilities.map(r => `<li>${r}</li>`).join('')
                    }</ul>`;
            } else if (job.details) {
                detailsHtml = `<p class="text-sm text-gray-600 dark:text-gray-400 mt-1">${job.details}</p>`;
            }

            jobDiv.innerHTML = `
                <span class="absolute -left-[39px] top-1 h-5 w-5 rounded-full border-4 border-white dark:border-slate-900 ${dotColor}"></span>
                <div class="flex flex-col sm:flex-row sm:justify-between sm:items-baseline">
                    <h4 class="text-lg font-bold text-gray-900 dark:text-gray-100">${job.company}</h4>
                    <span class="text-sm font-medium text-gray-500 dark:text-gray-400">${job.period}</span>
                </div>
                <p class="text-gray-700 dark:text-gray-300 italic">${job.position}</p>
                ${detailsHtml}
            `;
...
function renderTeaching() {
    ...
    if (teachingContainer && teaching.teaching) {
        ...
        teaching.teaching.forEach(course => {
            const li = document.createElement('li');
            li.className = "bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 p-3 rounded-lg shadow-sm";
            li.innerHTML = `
                <div class="flex justify-between">
                    <span class="font-bold text-gray-900 dark:text-gray-100">${course.course}</span>
                    <span class="text-xs font-semibold bg-gray-100 dark:bg-slate-700 text-gray-600 dark:text-gray-300 px-1.5 py-0.5 rounded">${course.semester}</span>
                </div>
                <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">${course.institution}</p>
            `;
    ...
    if (serviceContainer && teaching.service) {
        const reviewerHtml = teaching.service.reviewer ? `
            <div class="bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 p-4 rounded-lg shadow-sm">
                <p class="font-bold text-gray-700 dark:text-gray-200 mb-2">Conference Reviewer</p>
        ...
        const talksHtml = teaching.service.talks ? `
            <div class="bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 p-4 rounded-lg shadow-sm mt-3">
                <p class="font-bold text-gray-700 dark:text-gray-200 mb-2">Tutorials & Talks</p>
                <ul class="list-disc list-inside text-gray-600 dark:text-gray-400 text-sm space-y-1">
...
function filterPubs(category, btn) {
    document.querySelectorAll('.filter-btn').forEach(b => {
        b.classList.remove('bg-brand', 'text-white', 'shadow-sm', 'dark:bg-blue-600');
        b.classList.add('bg-white', 'text-gray-600', 'border', 'border-gray-200', 'dark:bg-slate-800', 'dark:text-gray-300', 'dark:border-slate-600');
    });
    btn.classList.remove('bg-white', 'text-gray-600', 'border', 'border-gray-200', 'dark:bg-slate-800', 'dark:text-gray-300', 'dark:border-slate-600');
    btn.classList.add('bg-brand', 'text-white', 'shadow-sm', 'dark:bg-blue-600');
...
function initTheme() {
    ...
    updateThemeToggleUI(shouldBeDark);
}
